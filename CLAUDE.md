...
docker-compose.yml    Docker Compose stack for dev + production droplets
...
| Layer    | Stack                                                                                                                                                                                                                                                                                                           |
| -------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Backend  | Node.js (ESM) · Express · Mongoose 7 · MongoDB · Redis (native TCP in production; Upstash REST in development) · JWT · bcryptjs · Multer · Cloudinary · Brevo (prod email) / Mailtrap (dev) · Firebase Admin (FCM) · pdfkit / pdf-lib · archiver · node-cron · pino (structured logs) + pino-http (request ids) |
...
- **File pipeline:** Android uploads via multipart → Cloudinary public*id `MyMediVault/h*{hospitalId}/p*{patientMongoId}/{folder_slug}/{date}*{hash}`. Files may be `public`or`signed` (5-min TTL). Thumbnails 120×120 for images.
...
- **Redis** — `REDIS_URL` drives the production droplet's local Redis service; `UPSTASH_REDIS_REST_URL/TOKEN` switches the client to Upstash for development and other hosted environments. If neither is set, the backend falls back to an in-memory Map-based store with real TTL semantics; that fallback is per-process and should stay dev-only because it can silently lose OTPs / partial-reg state across replicas.
- **Compression sidecar** — `USE_COMPRESSION_SERVICE`, `COMPRESSION_SERVICE_URL`, `COMPRESSION_SERVICE_SECRET` (shared with sidecar's `INTERNAL_API_SECRET`). **Mandatory in prod (TD-D4, 2026-04-25):** [config/env.js](backend/src/config/env.js) refuses to boot when `NODE_ENV=production` AND `USE_COMPRESSION_SERVICE !== "true"`. The in-process pdf-lib fallback OOMs at scale on large patients; the sidecar is the only safe production path. The production deployment already has it set to `true`. To turn it off in prod requires either swapping the merge path to a natively-streaming Node implementation or accepting the OOM risk for small deployments only.
- **Security** — `JWT_SECRET`, `REFRESH_TOKEN_SECRET` (64+ chars, no `dev-` prefix in prod, MUST be distinct random strings). Helmet explicitly configures `crossOriginOpenerPolicy` to `same-origin`.
...
- **`FileLogger` writes on-device logs in release builds** (2-day retention at `Android/data/com.hospital.management/files/logs/`). `X-Hospital-Id` is redacted; `Authorization`/`Cookie`/`Set-Cookie` are also redacted. D/I-level messages are gated behind `verboseFileLog` (default off) — only W/E/WTF persist by default. `FileLogger.rotate()` is called on logout to archive the current session's log so the next user cannot read it. Disclose in Data Safety ("Diagnostics / Crash logs") OR tighten further via [TD-A08](docs/audit/06-tech-debt-ledger.md) (now shipped).
...
- 🛠️ ~~**Jetpack Compose is dependency-loaded but unused.**~~ RESOLVED (TD-A06). All 7 dead deps removed: Compose (8 entries + `composeOptions` + `buildFeatures.compose`), CameraX (4), DataStore, Coil, iText7, Accompanist, Shimmer. Corresponding proguard rules and `shimmer_*` colour tokens also cleaned. Every screen remains ViewBinding XML.
...
- 🛠️ ~~**`AuthInterceptor` classifies 401s by substring-matching the response body**~~ — RESOLVED (TD-A07). Every backend 401 now includes a top-level `errorCode` field. `AuthInterceptor.kt` parses `JSONObject(body).optString("errorCode")` first, then falls back to substring matching for one release cycle. **Stable error codes:** `SESSION_CONFLICT` (revoked by another login), `ACCOUNT_DISABLED` (admin disabled account), `AUTH_CODE_REQUIRED` (7-day re-verify), `SESSION_EXPIRED` (idle/expired session), `TOKEN_INVALID` (malformed/expired JWT), `TOKEN_MISSING` (no token at all), `INVALID_CREDENTIALS` (wrong password/authCode), `CHALLENGE_EXPIRED` / `NO_BIOMETRIC_KEY` / `KEY_PARSE_FAILED` / `INVALID_SIGNATURE` (biometric flow), `INVALID_AUTH_CODE` (wrong Auth Code on reverify). Rewording any `message` string on the server no longer breaks the Android classifier.
