 ...
 *
 * Verifies the 3 notification-gating flows by stubbing Firebase Admin +
 * Hospital.findById, then calling the actual push.service functions.
 *
 * Flows tested:
 *   1. notifyNewLogin — new device sign-in
 *   2. notifyPasswordChanged — password changed
 *   3. notifyAdminsOfDeletionRequest — hospital requests deletion
 *
 * Each flow is tested with prefs = ON, OFF, and absent (defaults). The test
 * asserts both the GATING (opted-out users are skipped) and the PAYLOAD
 * (title, body, data.type).
 *
 * Usage:
 *   node scripts/test-fcm-notifications.js
 *
 * This is a mock-based integration test — it never contacts real FCM, so
 * it works without FIREBASE_* env vars.
 */
...
//    (admin.messaging is a getter — can't replace the function itself,
//     but we can swap `.send` on the returned singleton instance.)
// ---------------------------------------------------------------------------
...
{
  // opted IN + has token → expect one push with NEW_LOGIN payload
  ...
  log(
    sentMessages[0]?.notification?.title === "New Sign-in",
    "title = New Sign-in",
    sentMessages[0]?.notification?.title,
  );
  log(
    sentMessages[0]?.data?.type === "NEW_LOGIN",
    "data.type = NEW_LOGIN",
  );
  log(
    sentMessages[0]?.token === "device-token-user-1",
    "token routed to correct device",
  );

  // opted OUT → expect NO push
  sentMessages.length = 0;
  stubFindById({
  ...
  // no prefs at all → default on → should attempt push
  sentMessages.length = 0;
...
console.log("\nFlow 2 — notifyPasswordChanged");
{
  ...
  log(r1.success === true, "opted-in → push sent");
  log(
    sentMessages[0]?.data?.type === "PASSWORD_CHANGED",
    "data.type = PASSWORD_CHANGED",
  );
  log(
    sentMessages[0]?.notification?.title === "Password Changed",
...
{
  sentMessages.length = 0;
  // 3 admins: 2 opted-in with tokens, 1 filtered by query (opted-out)
  stubFindAdmins([
  ...
  log(
    sentMessages.every((m) => m.data.type === "DELETION_REQUEST"),
    "all messages have data.type = DELETION_REQUEST",
  );
  log(
    sentMessages[0]?.notification?.body?.includes("St Mary's Hospital"),
    "body contains hospital name",
  );

  // No opted-in admins → empty array → 0 pushes
  sentMessages.length = 0;
  stubFindAdmins([]);
  ...
  // Admin without token → skipped silently
  sentMessages.length = 0;
