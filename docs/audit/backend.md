...
| Services | `mail.service.js:sendLogoutConfirmationEmail`, `token.service.js:cleanupExpiredSessions` are orphaned exports | `01-dead-code.md` §D |
...
**Total: 59 endpoints.** Corrections vs prior audit shown in **bold**.
...
| POST | `/api/auth/login/resend-auth-code` | otpLimiter, verifyTempToken | tempToken AUTH_CODE |
...
| GET | `/api/patients` | supports search, pagination |
...
### Export — 3, Audit — 2, Admin — 2, Version — 1, Notifications — 3, Health — 2

Unchanged from prior audit; see [routes/](../../backend/src/routes/) for exact lines.
...
**Backend:** 1 test file — [__tests__/auth.controller.test.js](../../backend/src/__tests__/auth.controller.test.js). No integration tests of session flow, patient CRUD, export, compression, or admin force-delete.
