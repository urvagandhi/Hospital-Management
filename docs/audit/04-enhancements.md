...
Ground truth: one test file exists — [backend/src/**tests**/auth.controller.test.js](../../backend/src/__tests__/auth.controller.test.js). No frontend tests, no sidecar tests.
...
| **Redis**               | OTP store, biometric nonces, rate limiter counters | OTPs fail to verify; rate limits reset           | In-memory Map auto-fallback ([redis.service.js](../../backend/src/services/redis.service.js)) | Delayed — rate limits effectively disabled per-instance | None            |
...
| **FAIL-002** | ✅ SHIPPED 2026-04-25 (`a1bd66e`) | —   | ~~In-memory Redis fallback silently activates in prod.~~ Production now uses native Redis via `REDIS_URL`; the in-memory fallback still exists as a per-process degraded mode if Redis becomes unavailable, so production must monitor for that condition. |
...
4. ~~In-memory Redis fallback activates silently in prod~~ — RESOLVED 2026-04-25 (`a1bd66e`). Production now uses native Redis via `REDIS_URL`; Upstash remains a dev/hosted option.
...
_Proceed to `05-claude-md-update.md` for the CLAUDE.md delta, and `06-tech-debt-ledger.md` for the prioritised backlog._
...
| **AND-014** | 🟡 Medium | M   | **No certificate pinning** — deliberately removed because the production host can rotate certificates during deployment / TLS renewal ([network_security_config.xml:11-13](../../android-app/app/src/main/res/xml/network_security_config.xml)). System-CA trust is the fallback. If the cert rotation policy gets pinned to a specific CA in the future, re-enable pinning via backup-pin strategy.                                                                                                                                      |
...
_Proceed to `06-tech-debt-ledger.md` for the prioritised Android backlog (section "Android backlog — `TD-A01`..`TD-A15`")._
